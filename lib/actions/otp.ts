...
export async function validateAndSendOTP(contractId: string, enteredEmail: string, lang: string = 'en') {
    ...
    const signingUrl = `${baseUrl}/${lang}/sign/${contractId}?email=${encodeURIComponent(enteredEmail)}`

    const { error: otpError } = await supabase.auth.signInWithOtp({
        email: enteredEmail,
        options: {
            shouldCreateUser: true,
            emailRedirectTo: `${baseUrl}/${lang}/auth/callback?next=${encodeURIComponent(signingUrl)}`,
        }
    })
