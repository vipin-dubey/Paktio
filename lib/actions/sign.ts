...
export async function submitSignature(contractId: string, signerInfo: SignerInfo, lang: string = 'en') {
            ...
            const emailPromises = Array.from(emailAddresses).map(email =>
                resend.emails.send({
                    ...
                    html: `
                        ...
                        <p><a href="${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/${lang}/history/${contractId}">View Contract History</a></p>
                    `
                })
            )
    ...
    revalidatePath(`/${lang}/history/${contractId}`)
